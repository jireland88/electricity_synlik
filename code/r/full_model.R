...
obs_ord <- cbind(rep(1, obs_length),
                 poly(obs[order(obs)], 4))
                 
#obs_ord_diff <- cbind(rep(1, obs_length - 1),
#                      ordered_differences(obs))

spike_series_obs <- spike_series(obs)
spike_series_obs_ord <- cbind(rep(1, obs_length),
                              spike_series_obs[order(spike_series_obs)])
